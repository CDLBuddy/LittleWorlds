// src/game/GameApp.ts - Main game application orchestrator
...
export class GameApp {
  ...
  private uiSticksEnabled = false;
  private uiMove = { x: 0, y: 0, active: false };
  ...
  constructor(
    ...
    startParams: { roleId: RoleId; areaId: AreaId; fromArea?: AreaId }
  ) {
    ...
    this.eventUnsubscribe = this.bus.on((event) => {
      ...
      // ---------------------------------------------------------------------
        ...
        if (nextActive !== this.uiLookActive) {
          this.uiLookActive = nextActive;
          this.cameraRig?.setUserControlEnabled(!this.uiLookActive);
        }
  ...
  private onCompanionCall() {
    ...
    if (interactable) {
      ...
      console.log(
        '[GameApp] Available interactables:',
        this.interactables.map((i) => i.id)
      );
    }
  }
  ...
  async start() {
    ...
    if ('registerDynamic' in world && typeof world.registerDynamic === 'function') {
      (world.registerDynamic as (callback: (interactable: { id: string; mesh: AbstractMesh; interact: () => void; dispose: () => void }) => void) => void)(
        (interactable: { id: string; mesh: AbstractMesh; interact: () => void; dispose: () => void }) => {
          this.interactionSystem!.registerInteractable(interactable);
        }
      );
    }
    ...
    this.progressionSystem = new ProgressionSystem(this.taskSystem, startRole, this.startParams.areaId, {
    ...
    this.autosaveSystem = new AutosaveSystem(this.bus, this.taskSystem, startRole, this.startParams.areaId);
    ...
    if (import.meta.env.DEV) {
      ...
      if (this.taskSystem && this.currentWorld && this.progressionSystem && this.autosaveSystem) {
        setSnapshotRefs(this.taskSystem, this.currentWorld, this.progressionSystem, this.autosaveSystem as unknown as { roleId?: 'boy' | 'girl' });
      }
      ...
      (window as { __traceDump?: (count?: number, category?: 'event' | 'save' | 'switch' | 'system' | 'error') => unknown }).__traceDump = (
        count?: number,
      ...
      (window as { givefind?: (areaId: string, findId: string) => void }).givefind = (areaId: string, findId: string) =>
        cheatSystem.giveFind(areaId, findId);
      (window as { setfindcount?: (areaId: string, count: number) => void }).setfindcount = (areaId: string, count: number) =>
        cheatSystem.setFindCount(areaId, count);
  ...
  private onRestart() {
    ...
    if (this.taskSystem) {
      ...
      this.progressionSystem = new ProgressionSystem(this.taskSystem, this.startParams.roleId, this.startParams.areaId, {
