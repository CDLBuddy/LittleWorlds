...
export class WorldEditor {
  ...
  /** Generate stable UUID for markers */
  ...
  enable() {
    ...
    this.keyObserver = this.scene.onKeyboardObservable.add((kbInfo) => {
      if (kbInfo.type === KeyboardEventTypes.KEYDOWN && kbInfo.event) {
        ...
        // Export layout (Ctrl+E or Ctrl+Shift+C)
        if (kbInfo.event.ctrlKey && (key === 'e' || (kbInfo.event.shiftKey && key === 'c'))) {
          kbInfo.event.preventDefault();
          this.exportLayout();
          return;
        }

        // Export JSON layout (Ctrl+Shift+E)
        if (kbInfo.event.ctrlKey && kbInfo.event.shiftKey && key === 'e') {
          kbInfo.event.preventDefault();
          this.exportLayoutJSON();
          return;
        }
  ...
  private handlePlaceMarker(pickInfo: PickingInfo | null) {
    ...
    const markerData: PositionMarker = { 
      id: this.generateId(),
      mesh: marker, 
      position, 
      rotYRad: rotation, 
  ...
  private handleMarkerTransform(key: string, isShift = false) {
    ...
    switch (key.toLowerCase()) {
      ...
      case 'f':
        ...
        break;
      case 'q':
        this.selectedMarker.rotYRad -= rotateSpeed;
        this.selectedMarker.mesh.rotation.y = this.selectedMarker.rotYRad;
        this.addToHistory({
          type: 'rotate',
          snapshot: this.createSnapshot(this.selectedMarker),
          oldSnapshot
        });
        this.updateStatus(`Rotation: ${this.selectedMarker.rotYRad.toFixed(2)}`);
        return;
      case 'e':
        this.selectedMarker.rotYRad += rotateSpeed;
        this.selectedMarker.mesh.rotation.y = this.selectedMarker.rotYRad;
  ...
  private exportLayout() {
    ...
    const suggestedPath = `src/game/worlds/layouts/${this.areaId}Layout.ts`;
 ...
 * METADATA:
 * - Area ID: ${this.areaId}
 * - Marker Count: ${this.markers.length}
 * - Generated: ${timestamp}
...
`;
    ...
    Object.keys(byCategory).forEach(category => {
      ...
      if (items.length > 0) {
        ...
        items.forEach(m => {
          ...
          output += `    rotYRad: ${m.rotYRad.toFixed(2)},\n`;
  ...
  /** Export layout as JSON for diff-friendly review */
  private exportLayoutJSON() {
    ...
    console.log('[WorldEditor] Layout JSON:');
    console.log(output);

    // Copy to clipboard
    if (navigator.clipboard) {
      void navigator.clipboard.writeText(output).then(() => {
        console.log('[WorldEditor] ✓ JSON layout copied to clipboard!');
        this.updateStatus(`✓ Exported ${this.markers.length} markers as JSON`);
      });
    } else {
      this.updateStatus(`JSON exported (${this.markers.length} markers)`);
    }
  }
  ...
  private recreateMarker(snapshot: MarkerSnapshot): PositionMarker {
    ...
    const marker = MeshBuilder.CreateSphere(`marker_${this.markerCount++}`, { diameter: 0.5 }, this.scene);
    marker.position = position;
    marker.rotation.y = snapshot.rotYRad;
  ...
  private handleDuplicate() {
    ...
    const rotation = this.selectedMarker.rotYRad;
    ...
    const markerData: PositionMarker = { 
      ...
      rotYRad: rotation, 
      label, 
      category: this.selectedMarker.category 
    };
    this.markers.push(markerData);
    this.addToHistory({ 
      type: 'place', 
  ...
  private undo() {
    ...
    if (entry.type === 'place') {
      ...
      const index = this.markers.findIndex(m => m.id === entry.snapshot.id);
      if (index >= 0) {
      ...
      if (marker) {
        ...
        marker.rotYRad = entry.oldSnapshot.rotYRad;
        marker.mesh.rotation.y = marker.rotYRad;
      }
  ...
  private redo() {
    ...
    if (entry.type === 'place') {
      ...
      if (marker) {
        ...
        marker.mesh.position.copyFrom(marker.position);
        marker.rotYRad = entry.snapshot.rotYRad;
        marker.mesh.rotation.y = marker.rotYRad;
      }
    ...
    try {
      ...
      data.markers?.forEach((m) => {
        const snapshot: MarkerSnapshot = {
          id: m.id || this.generateId(), // Generate ID for legacy sessions
          position: m.position,
          rotYRad: m.rotYRad ?? m.rotation ?? 0, // Support both new and legacy format
          label: m.label,
          category: m.category
        };
  ...
  private createCommandPanel() {
    ...
    this.commandsDiv.innerHTML = `
      ...
      <div><span style="color: #fff;">Ctrl+Shift+C:</span> Export layout (alt)</div>
      <div><span style="color: #fff;">Ctrl+Shift+E:</span> Export JSON</div>
  ...
  private getCategoryColor(category: 'tree' | 'interactable' | 'prop' | 'general'): Color3 {
    switch (category) {
      case 'tree': return new Color3(0, 1, 0); // Green
      case 'interactable': return new Color3(0, 0, 1); // Blue
      case 'prop': return new Color3(1, 1, 0); // Yellow
